...
| `/leads/[id]`         | `LeadDetailClient`        | Detalhe + edit (`PATCH /leads/:id`)               |
| `/empresas`           | `CompaniesClient`         | Lista + modal create (`POST /companies`)          |
| `/empresas/[id]`      | `CompanyDetailClient`     | Detalhe + edit (`PATCH /companies/:id`)           |
| `/contatos`           | `ContactsClient`          | Lista + modal create (`POST /contacts`)           |
| `/contatos/[id]`      | `ContactDetailClient`     | Detalhe + edit (`PATCH /contacts/:id`)            |
| `/produtos`           | `ProductsClient`          | Lista + modal create (`POST /products`)           |
| `/produtos/[id]`      | `ProductDetailClient`     | Detalhe + edit (`PATCH /products/:id`)            |
| `/servicos`           | `ServicesClient`          | Lista + modal create (`POST /services`)           |
| `/servicos/[id]`      | `ServiceDetailClient`     | Detalhe + edit (`PATCH /services/:id`)            |
| `/tarefas`            | `TasksClient`             | Lista + modal create (`POST /tasks`)              |
| `/tarefas/[id]`       | `TaskDetailClient`        | Detalhe + edit (`PATCH /tasks/:id`)               |
| `/oportunidades`      | `OpportunitiesClient`     | Lista + modal create (`POST /opportunities`)      |
| `/oportunidades/[id]` | `OpportunityDetailClient` | Detalhe + edit (`PATCH`); won/lost                |
| `/funil`              | `FunilClient`             | Kanban HTML5 DnD → `POST /opportunities/:id/move` |

Shell Ember: create/detail/edit nas entidades CRM principais; stubs Sistema/SaaS/Insights com `PageHeader` + `StatusBadge`.
